...
* Use `receive_full` to obtain full details about an incoming message: receiving signal strength and the device's `current_time` when the message arrived.
...
.. py:function:: receive_full()
    ...
    * a timestamp: the value returned by `microbit.current_time` when the message was receieved.

    For example::

        msg, rssi, timestamp = radio.receive_full()
